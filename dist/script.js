...
const modals = () => {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    const trigger = document.querySelectorAll(triggerSelector),
      ...
      // получаем все модальные окна со страницы, чтобы потом их закрыть 
      scroll = calcScroll();
  ...
  function calcScroll() {
    ...
    // как вычислить размер прокрутки = полная ширина - ширина контента внутри без прокрутки = сама прокрутка
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  ...
  header.addEventListener('click', e => {
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      // отделяем точку, от эл., который пришел в функцию(заменяем на пустую строку) --> .class => class
      tab.forEach((item, i) => {
...
// скрипты начинают выполнятся когда готова dom структура
window.addEventListener('DOMContentLoaded', () => {
  "use strict";

  let modalState = {};
  let deadline = '2026-12-18';
